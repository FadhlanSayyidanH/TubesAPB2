...
void _thickSegment(img.Image image, double x1, double y1, double x2, double y2,
    double halfWidth, img.Color color) {
  ...
  img.fillCircle(image, x: x1.round(), y: y1.round(), radius: halfWidth.round(), color: color);
  img.fillCircle(image, x: x2.round(), y: y2.round(), radius: halfWidth.round(), color: color);
}

// UNTUK APA: menggambar tanda centang (✓) — lambang "hadir" pada absensi —
// dari dua coretan: garis pendek turun lalu garis panjang naik.
void _drawCheck(img.Image image, int cx, int cy, double scale, img.Color color) {
  final hw = 30 * scale;
...
void main() {
  ...
  stdout.writeln('Ikon dibuat: assets/icon/app_icon.png & app_icon_foreground.png');
}
